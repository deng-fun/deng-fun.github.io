...
export const HomeSiderSegmented = ({ className, defaultValue, ...props }: HomeSiderSegmentedProps) => {
  const $value = useStore(homeSiderSegmentType);

  // 初始化时设置默认值
  useEffect(() => {
    if (defaultValue && $value !== defaultValue) {
      homeSiderSegmentType.set(defaultValue);
    }
  }, [defaultValue]);
